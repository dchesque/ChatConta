...
export const FornecedorSelectorModal: React.FC<FornecedorSelectorModalProps> = ({
  ...
  useEffect(() => {
    const loadCredores = async () => {
      try {
        ...
        console.log('Credores carregados:', data);
        setCredores(data || []);
      } catch (error) {
  ...
  const credoresFiltrados = credores.filter(credor =>
    credor.name.toLowerCase().includes(busca.toLowerCase()) ||
    credor.document?.includes(busca) ||
    credor.email?.toLowerCase().includes(busca.toLowerCase())
  );
          ...
          <div className="max-h-96 overflow-y-auto space-y-2">
            {loading ? (
              <div className="text-center py-8">
                ...
                <p className="mt-2 text-muted-foreground">Carregando credores...</p>
              </div>
            ) : credoresFiltrados.length === 0 ? (
              <div className="text-center py-8">
                <Building2 className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
                <p className="text-muted-foreground">
                  {busca ? 'Nenhum credor encontrado' : 'Nenhum credor cadastrado'}
                </p>
              </div>
            ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="font-medium text-gray-900">{credor.name}</h3>
                        {credor.notes && (
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        <span>{credor.document}</span>
                        {credor.email && <span>{credor.email}</span>}
                        {credor.phone && <span>{credor.phone}</span>}
                      </div>
